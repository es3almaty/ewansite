...
CAPABILITY CLAIM: For each assessment, identify what specific competence the institution is claiming this assessment certifies.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { statusCode: 502, headers, body: JSON.stringify({ error: "Analysis service temporarily unavailable. Please try again in a moment." }) };
    }
    ...
    try {
      ...
      return { statusCode: 502, headers, body: JSON.stringify({ error: "Could not parse the analysis. The document may not contain assessments in a readable format." }) };
    }

    if (!result.assessments || !Array.isArray(result.assessments) || result.assessments.length === 0) {
      return { statusCode: 422, headers, body: JSON.stringify({ error: "No assessments found in this document. Please check the file is a syllabus or module handbook with formal assessments listed." }) };
    }
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, result, submittedBy: { email, institution: institution || null } }),
    };
  } catch (err) {
